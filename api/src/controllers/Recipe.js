...
function addRecipe(req, res, next) {
  ...
  if (!title || !summary)
    return res.send({
      error: 500,
      message: "You need a name and summary",
    });
  Recipe.create({
    id: uuidv4(),
    title: title,
    ...
    .then((newRecipe) => {
      return res.json({
        message: "A new recipe has been created!",
      });
    })
  ...
  if (!q) {
    try {
      const recipeApi = await axios.get(
        `https://api.spoonacular.com/recipes/complexSearch?addRecipeInformation=true&apiKey=${YOUR_API_KEY}&number=100`
      );
      ...
      return res.send(recipeDBres.concat(recipeApiRes));
    } catch (err) {
      next(err);
    }
  } else {
    ...
    try {
      ...
      const filteredRecipeApi = await infoNeededApi2.filter((recipe) =>
      ...
      Promise.all([filteredRecipeApi, recipeDB]).then((respuesta) => {
        let [filteredRecipeApi, recipeDBres] = respuesta;
        return res.send(recipeDBres.concat(filteredRecipeApi));
      });
    } catch (err) {
      next(err);
    }
...
async function getRecipeById(req, res) {
  try {
    const response = await axios.get(
      `https://api.spoonacular.com/recipes/${req.params.id}/information?apiKey=${YOUR_API_KEY}`
    );
    ...
    if (error.response?.status === 404) {
      ...
      res.status(500).json({ error: "Error to access!" });
    }
  }
}
